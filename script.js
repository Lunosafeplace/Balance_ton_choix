...
if (window.location.pathname.endsWith('index.html')) {
    ...
    function afficherInfosSupplementaires(infos, estCorrect) {
        if (!estSurCelebrities) {
            infosTexte.textContent = infos;
            infosComplementaires.innerHTML = "";

            // Ajout de cette partie pour afficher "Correct" ou "Faux"
            const resultatMessage = estCorrect ? "<span style='color: green;'>Correct !</span>" : "<span style='color: red;'>Faux !</span>";
            infosComplementaires.innerHTML += `<p>Résultat : ${resultatMessage}</p>`;
    ...
    function afficherQuestionSuivante() {
        ...
        if (questionsPosees.length < celebrites.length) {
            ...
            questionPage.innerHTML = 'Oups, tu as vu toutes les célébrités enregistrées à ce jour...';
        }
    }

    function verifierChoix(choix) {
        const estCorrect = choix === celebrites[indexCelebriteActuelle].reponse;
        if (estCorrect) {
            reponsesCorrectesConsecutives++;
        } else {
            reponsesCorrectesConsecutives = 0;
        }
        afficherInfosSupplementaires(celebrites[indexCelebriteActuelle].infosSupplementaires, estCorrect);
        afficherReponsesConsecutives();
    }
    ...
    if (demarrerQuizBtn) {
        ...
        console.error("L'élément demarrer-quiz n'a pas été trouvé.");
    }
    ...
    if (demarrerJeuBtn) {
        demarrerJeuBtn.addEventListener('click', () => {
            ...
            explicationPage.style.display = 'block';
        });
    } else {
...
if (estSurCelebrities) {
    ...
    function afficherCelebrities(celebrities) {
        celebritiesList.innerHTML = ''; // Efface la liste existante
        celebrities.forEach(celebrity => {
            ...
            celebrityDiv.innerHTML = `
                ...
                <h3>${celebrity.nom}</h3> 
            `;
            console.log(celebrityDiv);
            celebritiesList.appendChild(celebrityDiv);

            // Ajout d'un écouteur d'événements sur la photo
    ...
    function filtrerCelebrities(texteRecherche) {
        ...
        afficherCelebrities(celebritiesFiltrees);
    }
    ...
    // Affichage initial de toutes les célébrités
    afficherCelebrities(celebrites);

    // Définition des éléments pour celebrities.html
    const infosPage = document.getElementById('infos-page');
    const infosTexte = document.getElementById('infos-texte');
    const infosComplementaires = document.getElementById('infos-complementaires');
